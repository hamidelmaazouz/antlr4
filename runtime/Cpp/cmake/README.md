...
ANTLR_${name}_HEADER_OUTPUT_DIR - the output directory for generated header files
```
...
* `HEADER_OUTPUT_DIRECTORY` - the output directory for the generated header files, passed to the ANTLR tool as `-header-dir`. By default headers go to the output directory. It needs an ANTLR tool that supports `-header-dir`
...
To keep the generated headers apart from the generated `.cpp` files, so that only the headers are on the include path:

```cmake
find_package(ANTLR REQUIRED)
antlr_target(Sample T.g4 HEADER_OUTPUT_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/include)
add_executable(demo main.cpp ${ANTLR_Sample_CXX_OUTPUTS})
target_include_directories(demo PRIVATE ${ANTLR_Sample_HEADER_OUTPUT_DIR})
```
