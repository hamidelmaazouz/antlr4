...
All of them go to the same directory by default. To write the header files to a separate directory, for example a public include directory, use the `-header-dir` option. Add that directory to the include path when compiling the generated `.cpp` files. With CMake, `ANTLR_TARGET` accepts the same setting as `HEADER_OUTPUT_DIRECTORY`, see [its README](../runtime/Cpp/cmake/README.md), and `antlr4_generate` takes it as an optional argument, see [the package documentation](../runtime/Cpp/cmake/Antlr4Package.md).

```
$ antlr4 -Dlanguage=Cpp -o gen/src -header-dir gen/include MyGrammar.g4
```
