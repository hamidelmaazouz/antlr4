...
public class BuildDependencyGenerator {
    ...
    public List<File> getGeneratedFileList() {
        ...
        if (generator.getTarget().needsHeader()) {
          files.add(getHeaderOutputFile(generator.getRecognizerFileName(SourceType.HEADER)));
        }
        ...
        if ( g.isCombined() ) {
            ...
            // TLexer.h
            if (generator.getTarget().needsHeader()) {
                ST headerExtST = generator.getTemplates().getInstanceOf("headerFileExtension");
                String header = g.name + suffix + headerExtST.render();
                files.add(getHeaderOutputFile(header));
            }
        }
        ...
        if ( g.tool.gen_listener ) {
          ...
          if (generator.getTarget().needsHeader()) {
            files.add(getHeaderOutputFile(generator.getListenerFileName(SourceType.HEADER)));
          }
          ...
          if (generator.getTarget().needsHeader()) {
            files.add(getHeaderOutputFile(generator.getBaseListenerFileName(SourceType.HEADER)));
          }
        ...
        if ( g.tool.gen_visitor ) {
          ...
          if (generator.getTarget().needsHeader()) {
            files.add(getHeaderOutputFile(generator.getVisitorFileName(SourceType.HEADER)));
          }
          files.add(getOutputFile(generator.getVisitorFileName(SourceType.SOURCE)));

          // add generated base visitor; e.g., TBaseVisitor.java
          if (generator.getTarget().needsHeader()) {
            files.add(getHeaderOutputFile(generator.getBaseVisitorFileName(SourceType.HEADER)));
          }
...
//				files.add(new File(outputDir, fname));
	...
	public File getOutputFile(String fileName) {
		return getOutputFile(fileName, tool::getOutputDirectory);
	}

	/** Like {@link #getOutputFile(String)}, but for a header file, which
	 *  goes to the -header-dir directory when that option is set.
	 *  Without the option this calls {@link #getOutputFile(String)}.
	 */
	public File getHeaderOutputFile(String fileName) {
		if ( tool.headerOutputDirectory==null ) {
			return getOutputFile(fileName);
		}
		return getOutputFile(fileName, tool::getHeaderOutputDirectory);
	}

	private File getOutputFile(String fileName, Function<String, File> getOutputDirectory) {
		File outputDir = getOutputDirectory.apply(g.fileName);
		if ( outputDir.toString().equals(".") ) {
			// pay attention to -o then
			outputDir = getOutputDirectory.apply(fileName);
		}
		if ( outputDir.toString().equals(".") ) {
